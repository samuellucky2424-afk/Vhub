// Add this new action to smspool-service/index.ts to provide countdown data to frontend
...
if (action === 'get_order_countdown') {
    ...
    try {
        ...
        // 2. Check if order is still pending/active
        ...
        // 3. Check if countdown is already running
        ...
        let countdownData = {
            active: true,
            total_duration: 80000, // 80 seconds
            elapsed: 0,
            remaining: 80000,
            percentage: 0,
        ...
        if (countdownStarted) {
            ...
            // Update order metadata to mark countdown as started
        ...
        // 4. Check SMSPool status for real-time updates
        if (order.request_id) {
            try {
                ...
                if (data.status === 3 && data.sms) {
                    // OTP found - countdown should end
        ...
        log(`Countdown data for order ${order_id}: ${JSON.stringify(response.countdown)}`);
