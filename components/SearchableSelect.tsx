...
const SearchableSelect: React.FC<SearchableSelectProps> = ({
    ...
    const [failedImageValues, setFailedImageValues] = useState<Set<string>>(new Set());
    ...
    const renderIcon = (opt?: Option, showDefault = true) => {
        ...
        if (!opt) return null;
        const shouldShowImage = !!opt.iconUrl && !failedImageValues.has(opt.value);

        return (
            <div className="size-6 md:size-8 rounded-lg bg-slate-100 dark:bg-zinc-800 flex items-center justify-center shrink-0 overflow-hidden">
                {shouldShowImage ? (
                    <img
                        src={opt.iconUrl}
                        alt={opt.label}
                    ...
                    <span className="text-[10px] md:text-xs font-black tracking-[0.08em] text-slate-600 dark:text-slate-200">
                        {getFallbackLabel(opt.label)}
                    </span>
                )}
            </div>
        );
    ...
    return (
        <div className="relative group w-full" ref={wrapperRef}>
            ...
            <div
                ...
                className={`flex items-center gap-2 p-2.5 rounded-lg bg-slate-50 dark:bg-black/20 border transition-all cursor-pointer relative overflow-hidden ${isOpen ? 'border-primary ring-2 ring-primary/20' : 'border-slate-200 dark:border-zinc-800 hover:border-primary/50'
            ...
            >
                ...
                <div className="flex-1 min-w-0">
                    <p className="font-bold text-xs md:text-sm text-slate-900 dark:text-white truncate">
                        {selectedOption ? selectedOption.label : placeholder}
                    </p>
                    {(selectedOption?.subtitle || selectedLabel) && (
                        <p className="text-[10px] text-slate-500 truncate">{selectedOption?.subtitle || selectedLabel}</p>
                    )}
                </div>
