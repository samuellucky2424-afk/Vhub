...
const Dashboard: React.FC = () => {
  ...
  React.useEffect(() => {
    if (!sessionStorage.getItem('tempFixNoticeShown')) {
      setShowNotice(true);
      sessionStorage.setItem('tempFixNoticeShown', 'true');
    }
  }, []);
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-y-auto relative">
      ...
      <motion.header
        ...
        className="sticky top-0 z-20 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md px-4 md:px-8 py-4 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between gap-4"
      >
        <div className="flex-1 max-w-md">
          <motion.button
            ...
            className="w-full bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-full px-4 py-2 flex items-center justify-between gap-3 hover:border-primary transition-colors cursor-pointer"
          >
        ...
        {showNotice && (
          <>
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="fixed inset-0 m-auto z-[70] w-full max-w-md h-fit p-4 flex flex-col justify-center"
            >
              <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-xl overflow-hidden border border-slate-200 dark:border-zinc-800 p-6 relative">
                <button
                  onClick={() => setShowNotice(false)}
                  className="absolute top-4 right-4 size-8 rounded-full bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-zinc-700 flex items-center justify-center transition-colors"
                >
                  <span className="material-symbols-outlined text-sm">close</span>
                </button>
                ...
                <p className="text-slate-600 dark:text-zinc-600 text-sm leading-relaxed mb-6 font-medium">
                  This is a temporary fix. All numbers here are currently US-based. No other countries are available at the moment. As time goes on, other countries will be added.
                </p>
                <button
                  onClick={() => setShowNotice(false)}
                  className="w-full bg-primary hover:bg-primary/90 text-zinc-900 font-bold h-11 rounded-lg transition-all"
                >
                  I Understand
                </button>
              </div>
