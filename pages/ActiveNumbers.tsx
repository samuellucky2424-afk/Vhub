...
const ActiveNumbers: React.FC = () => {
    ...
    const syncOrderStatus = async (orderId: string) => {
        ...
        if (rawText) {
            try {
                ...
                throw new Error(`textverify-service returned invalid JSON (${response.status}): ${rawText}`);
            }
        }

        if (!response.ok) {
            const message = parsed?.message || parsed?.error || rawText || `textverify-service returned ${response.status}`;
    ...
    const checkStatus = async (orderId: string) => {
        ...
        try {
            const data = await syncOrderStatus(orderId);
        ...
        const syncPendingOrders = async () => {
            ...
            try {
                for (const item of pendingOrders) {
                    try {
                        const result = await syncOrderStatus(item.id);
    ...
    useEffect(() => {
        const updateCountdowns = () => {
            ...
            const newCountdowns: Record<string, { remaining: number; percentage: number; active: boolean }> = {};

            for (const order of pendingOrders) {
                const createdAt = new Date(order.createdAt).getTime();
                const elapsedMs = Date.now() - createdAt;
                const elapsedS = Math.floor(elapsedMs / 1000);
                const remaining = Math.max(0, COUNTDOWN_DURATION_S - elapsedS);
                const percentage = Math.min(Math.round((elapsedS / COUNTDOWN_DURATION_S) * 100), 100);
                const active = remaining > 0;

                newCountdowns[order.id] = { remaining, percentage, active };

                // When timer just hit 0, trigger an immediate status sync 
                // so the backend cancels on SMSPool and refunds.
                // We check `countdowns[order.id]?.active` to ensure we only trigger it ONCE
                // when transitioning from active (true) to expired (false).
                if (remaining === 0 && countdowns[order.id]?.active) {
                    console.log(`[Countdown] Order ${order.id} expired — triggering sync...`);
