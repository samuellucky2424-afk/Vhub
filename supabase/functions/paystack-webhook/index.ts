...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Extract user_id from top level OR from custom_fields
    let userId = metadata.user_id;

    if (!userId && metadata.custom_fields && Array.isArray(metadata.custom_fields)) {
      const userField = metadata.custom_fields.find((field: any) => field.variable_name === 'user_id');
      if (userField) {
    ...
    if (!userId) {
      console.error("[WEBHOOK ERROR] Missing user_id in metadata for wallet funding. Ref:", ref, "Metadata:", JSON.stringify(metadata));
      return new Response("OK", { status: 200 });
    }
