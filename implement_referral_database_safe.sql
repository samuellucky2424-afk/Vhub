-- ============================================================
-- SAFE REFERRAL SYSTEM DATABASE STRUCTURE IMPLEMENTATION
-- Run this in Supabase SQL Editor
-- ============================================================

-- 1️⃣ CREATE REFERRALS TABLE (IF NOT EXISTS)
...
-- 2️⃣ CREATE REFERRAL_USAGES TABLE (IF NOT EXISTS)  
...
-- 3️⃣ ENSURE ONE USER = ONE REFERRAL CODE (SAFE VERSION)
-- Add unique constraint on user_id in referrals table only if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (
...
-- 4️⃣ ENABLE ROW LEVEL SECURITY (RLS) - IF NOT ALREADY ENABLED
DO $$
BEGIN
    IF NOT EXISTS (
...
-- 5️⃣ CREATE RLS POLICIES FOR REFERRALS TABLE (DROP AND RECREATE)
...
-- 6️⃣ CREATE RLS POLICIES FOR REFERRAL_USAGES TABLE (DROP AND RECREATE)
...
-- 7️⃣ ENABLE REALTIME FOR REFERRAL TABLES (SAFE VERSION)
...
-- 8️⃣ CREATE INDEXES FOR PERFORMANCE (IF NOT EXISTS)
...
-- ✅ Tables created safely (IF NOT EXISTS)
-- ✅ One user = one referral code (unique constraint added safely)
